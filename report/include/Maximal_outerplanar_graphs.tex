...
\subsubsection{Partitioning a Binary Tree in Complete Binary Subtrees}
...
\begin{algorithm}[H]
	...
	\While{\texttt{VerticesDone} $\neq V(\mathcal{T})$}{
		...
		\ElseIf(\tcp*{Serialization $S(s_1,s_2,s_3)$}){$v$ is $S$ node}{
			...
			\If{$\nexists$ reachable layer}{$l \gets B$\texttt.{addLayer}$(l_{up},l_{low})$}
			\Else{$l \gets $ reachable layer either atop, below or inbetween $l(s_1)$ and $l(s_3)$}
