...
\begin{theorem}
	Any tree $T$ admits a optimal straight-line drawing, apart from a rounding error.
\end{theorem}
\begin{proof}
...
\begin{algorithm}[H]
	...
	\For{inner node $v$ of $T$}{
		\If{$v.\texttt{AmountOfChildren} > k$}{
			$k\gets v.\texttt{AmountOfChildren}$
		}
	$T'\gets T.\texttt{ExtendToCompletenessWithMarkedVertices}(\texttt{root},k)$
	}
	$\Gamma_{T'} \gets \texttt{Draw\_$k$-ary\_tree($T'$)}$\\
	$\Gamma_{T'}.\texttt{removeMarkedVertices}$\\
	\Return $\Gamma_{T'}$
\end{algorithm}
